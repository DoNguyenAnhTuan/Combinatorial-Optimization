...
# In hàng CM
total_sum_CM = sum(CM)
print(f"CM   {CM[0]}   {CM[1]}   {CM[2]}   {CM[3]}   {CM[4]}   {total_sum_CM}")
...
chosen_branch = f"X{row_labels[max_penalty_positions[0][1]]}{row_labels[max_penalty_positions[0][0]]}"
print(f" We choose {chosen_branch} to begin branch")

# Xóa hàng và cột theo chosen_branch
row_to_delete = max_penalty_positions[0][1]  # Chỉ số hàng
col_to_delete = max_penalty_positions[0][0]  # Chỉ số cột

# In hàng và cột cần xóa
print(f"\nXóa hàng: {row_labels[col_to_delete]} và cột: {row_labels[row_to_delete]}")

# Xóa hàng và cột từ ma trận RM
updated_RM = np.delete(RM_numeric, row_to_delete, axis=0)  # Xóa hàng
updated_RM = np.delete(updated_RM, col_to_delete, axis=1)  # Xóa cột

# In ma trận sau khi xóa
print("\nMa trận RM sau khi xóa:")
print("     A   B   D   E")  # In tiêu đề
for i in range(updated_RM.shape[0]):
    print(f"{row_labels[i + (1 if i >= row_to_delete else 0)]}  ", end="")  # Cập nhật chỉ số hàng
    for j in range(updated_RM.shape[1]):
        if isinstance(updated_RM[i, j], str) and updated_RM[i, j] == '×':
            print("×   ", end="")
        else:
            # Kiểm tra nếu giá trị không phải là NaN trước khi chuyển đổi
            if np.isnan(updated_RM[i, j]):
                print("×   ", end="")  # In ra '×' nếu là NaN
            else:
                print(f"{int(updated_RM[i, j])}   ", end="")
    print()

# Lấy giá trị tại hàng C (chỉ số 2) và cột D (chỉ số 3)
# Chú ý: Sau khi xóa hàng và cột, các chỉ số sẽ thay đổi.
# Lưu ý rằng row_to_delete và col_to_delete không thay đổi giá trị của updated_RM
value_CD = updated_RM[2, 3]  # Hàng C (sau khi xóa là hàng 1) và cột D (sau khi xóa là cột 2)

print(f"\nGiá trị tại vị trí hàng C và cột D là: {value_CD}")