...
# Build the tour in the number format
tour_string_num = ""
tour_string_alpha = ""
current_node = 0
...
for _ in range(n):
    for i, j in tour_edges:
        if i == current_node:
            # Add to numeric tour string
            tour_string_num += f"x{i+1}{j+1}->"
            # Add to alphabetic tour string
            tour_string_alpha += f"{mapping[i]}->"
            current_node = j
            break

# Remove the last '->' and add the total distance
tour_string_num = tour_string_num[:-2] + f" => z*={total_distance}"
tour_string_alpha = tour_string_alpha[:-2] + f" => z*={total_distance}"

# Output the formatted tours
print(tour_string_num)
print(tour_string_alpha)
